...
- 有界输入、fail-closed：请求契约恰好是 `{ window, budget }`；`budget` 必须是 `[1, 24]` 内的整数（schema 拒绝非整数，adapter 在任何阶段运行前拒绝越界值），任何未知请求键——例如模型自带的 `out_dir`——都会被拒绝而不是静默忽略，因此运行时所有的产物路径永远无法从模型侧被改写。
- Loader 表面上的真实成功/失败终态行为：合法调用返回结构化信封；畸形调用、越界输入、未知请求键、未知能力名都以真实注册工具产生的真实 `isError` 工具结果终止——全程没有手搓的回调。
...
| Surface | Role | Result |
|---|---|---|
| `RUN_BASELINE_PHYSICS` | 唯一注册的语义能力 | 带 provenance + `abstention` + `content_hash` 的类型化信封；无效/越界/未知请求返回 fail-closed 的 `isError` |
...
- 请求在语义边界 fail-closed：适配器在任何阶段运行前强制 `{ window, budget }` 契约、非空字符串 window、以及 `[1, 24]` 内的整数 budget——失败的调用不写产物、不伪造 provenance。未知请求键（包括任何模型自带的 `out_dir`）被拒绝而不是静默忽略，因此产物路径始终是 plugin config → 运行环境 → 模块默认。
