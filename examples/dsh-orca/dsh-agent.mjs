...
async function main() {
  ...
  // A startup timeout means NONE of the startup markers appeared in the
  // captured output within the timeout window. Concretely: the dsh-orca
  // plugin's own "[dsh-orca] plugin loaded" log was never written, so the
  // plugin did not finish loading; since "turn/start" is what actually
  // proves an LLM call was attempted, a startup timeout means no turn ever
  // began and no LLM call was made. The startup path (module load, Cordis
  // plugin composition, settings.yaml read) is model-agnostic, so retrying
  // with a different model would hit the same wall — surface it as a hard
  // failure before considering the provider fallback.
