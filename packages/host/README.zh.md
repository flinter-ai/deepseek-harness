---
description: "Web GUI Host 侧的包映射：HTTP 与 SPA 服务器、活动守卫、工作区目录选择实现、Git 源草稿发布器和插件清单投影。"
kind: "package-group"
---
...
`host/` 组提供 Web GUI 的普通 HTTP 服务器、服务已构建 Web 壳的 SPA dist 服务器、供部署生命周期策略使用的脱敏活动状态 writer、带原生／浏览／自适应组合包的工作区目录选择 seam、仅宿主的 Git/PR 源草稿发布器，以及只读的插件清单投影。这九个包都是产品包；浏览器传输位于 [`client/`](../client/README.zh.md)，组合应用是 [`apps/cli`](../../apps/cli/README.zh.md)，它启动 [`dsh-base` 组合包](../bundle/base/cordis.patch.yml) 来提供 `apps/web/` 下的 Web 应用。选择器后端可在共享 seam 后互相替换。
...
| [`idle-guard/`](idle-guard/README.zh.md) | 部署层 idle-stop controller 使用的脱敏活动状态 | 消费 `ctx.webServer`、`ctx.jobs`、`ctx.terminals` |
...
| [`source-publisher-github/`](source-publisher-github/README.zh.md) | 面向源草稿的仅宿主 Git worktree 与 GitHub Pull Request 发布器 | `ctx.sourcePublisher` |
