---
description: "Package map for the web GUI host half: the HTTP and SPA servers, activity guard, workspace-directory picking implementations, the Git source-draft publisher, and the plugin inventory projection."
kind: "package-group"
---
...
The `host/` group provides the web GUI's plain HTTP server, the SPA dist server that serves the built Web shell, the redacted activity state writer for deployment lifecycle policy, the workspace-directory picking seam with its native, browse, and adaptive composition packages, the host-only Git/PR source-draft publisher, and the read-only plugin inventory projection. All nine packages are product packages; the browser transport lives in [`client/`](../client/README.md), and the composed application is [`apps/cli`](../../apps/cli/README.md) booting the [`dsh-base` bundle](../bundle/base/cordis.patch.yml) that serves the web app under `apps/web/`. The picker backends replace one another behind the shared seam.
...
| [`idle-guard/`](idle-guard/README.md) | Redacted activity state for a deployment-owned idle-stop controller | consumes `ctx.webServer`, `ctx.jobs`, `ctx.terminals` |
...
| [`source-publisher-github/`](source-publisher-github/README.md) | Host-only Git worktree and GitHub Pull Request publisher for source drafts | `ctx.sourcePublisher` |
