...
`recordActivity()` 与 `activitySnapshot()` 为可选的 idle-stop 插件等宿主策略 提供脱敏生命周期接口。快照只包含最近一次已接受的应用活动时间、活动 HTTP handler 数量和活动 upgrade socket 数量，不包含请求数据、凭据或 route 名称； 宿主策略会把它与 job、PTY 数量合并后再执行生命周期操作。
