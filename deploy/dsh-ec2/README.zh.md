...
> **按需远程开发 — 无生产职责。** EC2 DSH Web 主机（`flinter-orca-dsh`）
> 作为默认停止的远程开发环境保留。此部署路径用于对该环境进行刻意的
> operator 部署；保持仅手动触发，绝不可在 push 时自动运行。它不是日常部署
> 通道 — 先启动主机，会话结束后由 idle-stop 告警将其停止。
