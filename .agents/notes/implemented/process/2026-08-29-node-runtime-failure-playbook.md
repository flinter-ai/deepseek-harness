...
| --- | --- | --- |
| Non-blocking / informational | Cloudflare PR preview deployment (permanently skipped; GitHub Pages is the canonical PR preview); real-provider E2E on fork PRs without provider secrets; Windows PR build/native/coverage/Wine; Python Windows/macOS release-shaped targets; Node 22.19 compatibility; observational and benchmark jobs | Record each as intentionally skipped or unavailable; do not make it a merge blocker or call it tested evidence. Reopen only through an explicit policy decision. |
| Important / blocking | Node 24 static, coverage, snapshots/artifacts; Node 26 compatibility; Python Linux SDK/release contract checks; aggregate verdict | These are the AWS/Linux PR gate. They must pass or have an explicit, reviewed infrastructure exception. |
| Current blocker | The preceding PR run reported Node 24 snapshots/artifacts failed; its failure log was unavailable until completion. The replacement run for the current head is re-running that gate. | Treat snapshots/artifacts as the only failure under investigation. Do not update snapshots or merge until the replacement run completes, the semantic diff is inspected, and the job passes or receives an explicit reviewed exception. |
