...
| Current blocker | Node 24 snapshots/artifacts is reported failed in the current PR run; GitHub has not released its failure log while the full CI run remains in progress | Treat this as the only current failure. Do not update snapshots or merge until the run completes, the semantic diff is inspected, and the job passes or receives an explicit reviewed exception. |
