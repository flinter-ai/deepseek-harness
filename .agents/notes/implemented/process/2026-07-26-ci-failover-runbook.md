...
Delete the `DSH_CI_FAILOVER_LINUX` or `DSH_CI_FAILOVER_WINDOWS` variable (or set it to anything other than `selfhosted`). New runs resolve back to the standard GitHub-hosted runners. Remove any extra instances that were registered during the incident.
