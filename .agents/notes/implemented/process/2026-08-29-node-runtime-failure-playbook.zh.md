...
| Current blocker | 前一个 PR 运行报告 Node 24 snapshots/artifacts 失败；其失败日志要等运行完成后才可获取。当前 head 的替代运行正在重新执行该门禁。 | 将 snapshots/artifacts 视为唯一正在调查的失败。在替代运行完成、检查语义差异且 job 通过或获得明确审查例外前，不更新 snapshot，也不合并。 |
