...
`CodeSandboxSdkRuntime`. It creates private, bounded VMs, maps the profile's
case-insensitive tier names to SDK `VMTier` values, connects with a write
session, executes one shell-quoted literal-argv transport, and shuts down the
VM after the command completes or is stopped. The SDK API token is host-only;
...
The profile suite and SDK runtime unit tests pass. A live no-credential E2E
created a private Pico sandbox, executed a literal argv payload, and shut it
down successfully. The API/CLI workspace probe also confirmed the managed
sandbox appears in the workspace after shutdown. No secret values are stored
in the repository or emitted by the tests.
