...
- **运行时自有的传输与 ancestry（`trace.js`）。** 回调 URL、HMAC 密钥以及七个 ancestry id（organization/project/episode/job/ir/jobOutput/artifact）外加 `runOrdinalBase` 与 `postTimeoutMs` 只从经过校验的插件配置或 `PES_TRACE_*` 环境解析——绝不来自工具/模型请求字段。URL/密钥只配其一、或开启传输却缺少 ancestry，都会在加载时 fail loud；两者都缺则发射器保持禁用。头部为 `x-dsh-signature`（T1 接缝选定的约定）。
