...
- 五个原型工具（`frames.sample`、`track.cotracker`、`boundary.detect`、`vlm.ask`、`artifact.write`）schema 保持稳定不变，由新测试钉住；确定性 stub 产物的 sha256 `content_hash` 与实际写入字节一致。
...
S0 是冻结的原型检查点：其中没有任何内容声称生产科学能力，镜像 smoke 仅是外部集成证据。下一项独立的 DSH 任务是在此之上实现语义能力适配器、最小能力注册表与契约测试；只有在该语义检查点通过后，才允许建立 integration 分支把 DSH 线与 `aws-runtime` 及其 AWS 附加能力结合，届时才可更新 `DSH_COMMIT`。本笔记须与已落地的 S0 事实（路径、schema、机制）在同一改动中保持同步更新。